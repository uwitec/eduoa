...
define ('_CONNECTION_ERROR_',"DB Connection error, please <a href=\"?go=config\">configure</a> your application.");
...
<!--
function GonxColor(el, BG1, BG2) {
	document.getElementById(el).style.background = (document.getElementById(el).style.background == BG1) ? BG2 : BG1;
}
//-->
</script>
<form method=get action=\"?\" name=bform><b>".$GONX["selectbackupfile"]." :</b><br/><br/>\n";
		...
		if ($i==0) {
			...
			*/
			...
			if ($page<$allpages)
			{
				$next = "<a href=\"?go=list&amp;page=".($page+1)."&orderby=$orderby\" class=tab-s>".$GONX["next"]."</a>";
			} else $next="";
			if ($page>1)
			{
				$prev = "<a href=\"?go=list&amp;page=".($page-1)."&orderby=$orderby\" class=tab-s>".$GONX["prev"]."</a>";
			} else $prev ="";
			$links = "";
			for ($i=0; $i<$allpages; $i++)
			{
				if (($i+1) == $page)
				{
					...
					$links .= "<a href=\"?go=list&amp;page=".($i+1)."&orderby=$orderby\" class=tab-g> ".($i+1)." </a>";
				}
			}
			/**
			* Order by
			*/
			$OrderMenu = "<select class=button OnChange=\"location.href='?go=list&page=$page&orderby='+ChgOrder.options[selectedIndex].value\" name=\"ChgOrder\">\n";
...
\t<tr>
			...
			foreach($GonxBackups as $k=>$v){
				...
				$db = $db[0];
				$res .= "\t<tr id=$k>
			<td><input type=\"radio\" name=\"bfile\" value=\"".$v['fname']."\" onclick=\"GonxColor($k,'khaki','#F6F6F6');\"></td>
			<th>$db</th>
			<td><font size=2px> <a href=\"?go=getbackup&bfile=".urlencode($v['fname'])."\" title=\"Download ".$v['fname']."\">".$v['fname']."</a></td>
			<td><em>size ".$v['fsize']." Ko</em></td>
			<td>".$v['mtime']."</td>
			<td>(<a href=\"?go=delete&fname=".$v['fname']."\" title=\"".$GONX["delete"]." ".$v['fname']."\" onclick=\"return ConfirmDelete();\"><img src=\"image.php?img=delete_gif\" border=0> ".$GONX["delete"]."</a>)</font><td>
</tr>\n\n";
			}
...
<th bgcolor=\"$color1\">Msg_text</font></th>
		...
		while ($line = $this->fetch_array($result)) {
	        ...
	        foreach ($line as $k=>$col_value) {
				if (!is_numeric($k)) {
					if ($col_value == "OK") {
					    $optimize = " <a href=\"?go=optimize&op=OPTIMIZE\" class=tab-s>Optimize DB</a>";
					} else $optimize="";
...
<input type=hidden name=go value=saveconfig>
	...
	function getbackup($bfile){
		if (is_file($this->backupdir."/".$bfile)) {
//		    $data = implode('',@file($this->backupdir."/".$bfile));
